#!/usr/bin/env python3
"""Media Tracker - Track movies and books with personalized recommendations."""
...
def display_movie(movie: Movie, show_id: bool = False):
    ...
    print(f"{id_str}{movie.title} ({movie.year or 'N/A'})")
    if movie.director:
...
def display_book(book: Book, show_id: bool = False):
    ...
    print(f"{id_str}{book.title}{year_str}")
    if book.author:
...
class MediaTracker:
    ...
    def update_movie_status(self):
        ...
        for movie in movies:
            print(f"[{movie.id}] {movie.title} - {movie.status.value}")
